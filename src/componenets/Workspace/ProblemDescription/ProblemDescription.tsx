...
import RectangleSkeleton from '@/componenets/Skeletons/RectangleSkeleton';
import CircleSkeleton from '@/componenets/Skeletons/CircleSkeleton';
...
const ProblemDescription:React.FC<ProblemDescriptionProps> = ({problem,_solved}) => {
	...
	const handleLike =async()=>{
		if (!user){
			toast.error('You must be logged into like a problem',{position:'top-left',theme:'dark'})
			return
		}
		if (updating) return
		setUpdating(true)
				...
				<div className='px-5'>
					...
					<div className='w-full'>
						...
						{!loading && currentProblem && (
							...
							<div className='flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 rounded p-[3px] 
							 ml-4 text-lg transition-colors duration-200 text-dark-gray-6'
							 onClick={handleLike}>
							...
							<div className='flex items-center cursor-pointer
							 hover:bg-dark-fill-3 space-x-1 rounded p-[3px] 
							  ml-4 text-lg transition-colors duration-200 text-green-s
							   text-dark-gray-6' onClick={handleDislike}>
								{disliked && !updating && <AiFillDislike className='text-dark-blue-s'/>}
								{!disliked && !updating && <AiFillDislike/>}
								{updating && <AiOutlineLoading3Quarters className='"animate-spin'/>}
								<span className='text-xs'>{currentProblem.dislikes}</span>
							</div>
							<div className='cursor-pointer hover:bg-dark-fill-3
							  rounded p-[3px]  ml-4 text-xl 
							  transition-colors duration-200 text-green-s
							   text-dark-gray-6 '
							   onClick={handleStar}>
